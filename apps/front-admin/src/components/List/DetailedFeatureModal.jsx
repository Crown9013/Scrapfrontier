...
const DetailedFeatureModal = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const data = row
          ? await axiosInterface('put', 'feature', formData, true)
          : await axiosInterface('post', 'feature', formData, true)
  ...
  const handleDelete = useCallback(async () => {
    axiosInterface('delete', `feature/${row._id}`, {})
      .then((res) => {
        if (res.status === 200) {
          refresh();
          closeModal();
      ...
      <Modal show={openModal} onHide={closeModal}>
        ...
        >
          ...
          <Modal.Body>
            ...
            {originImage && (
              <img
                src={`${process.env.REACT_APP_SERVER_URL}images/${originImage}`}
                alt="feautred"
                width={200}
                height={200}
                className="m-auto"
